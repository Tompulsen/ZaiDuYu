...
export class HunyuanApi implements LLMApi {
  path(): string {
    ...
    if (accessStore.useCustomConfig) {
      let baseUrl = accessStore.tencentUrl;
      if (baseUrl.endsWith("/")) baseUrl = baseUrl.slice(0, -1);
      if (!baseUrl.startsWith("http")) baseUrl = "https://" + baseUrl;
      return baseUrl;
    }

    const apiPath = ApiPath.Tencent;
    console.log("[Proxy Endpoint] ", apiPath);
    return apiPath;
  }
          ...
          async onopen(res) {
            ...
            if (
              ...
              try {
                ...
                console.debug("[Tencent] Failed to parse response as JSON:", error);
              }
